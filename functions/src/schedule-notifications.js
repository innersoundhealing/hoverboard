...
const scheduleNotifications = functions
  .region('europe-west1')
  .pubsub.topic('schedule-tick')
  .onPublish(async () => {
    ...
    const todayDay = moment()
      .utcOffset(notificationsConfig.timezone)
      .format('YYYY-MM-DD')
    ...
    if (schedule[todayDay]) {
      ...
      const usersIdsSnapshot = await firestore()
