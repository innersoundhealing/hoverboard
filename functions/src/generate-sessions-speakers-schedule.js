...
export const sessionsWrite = functions.region('europe-west4').firestore.document('sessions/{sessionId}').onWrite(async () => {
  return generateAndSaveData();
});

export const scheduleWrite = functions.region('europe-west4').firestore.document('schedule/{scheduleId}').onWrite(async () => {
  const scheduleConfig = functions.region('europe-west4').config().schedule;
...
export const speakersWrite = functions.region('europe-west4').firestore.document('speakers/{speakerId}').onWrite(async (change, context) => {
