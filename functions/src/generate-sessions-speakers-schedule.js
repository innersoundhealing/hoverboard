...
export const sessionsWrite = functions
  .region('europe-west1')
...
export const scheduleWrite = functions
  .region('europe-west1')
  .firestore.document('schedule/{scheduleId}')
  .onWrite(async () => {
...
export const speakersWrite = functions
  .region('europe-west1')
  .firestore.document('speakers/{speakerId}')
...
async function generateAndSaveData(changedSpeaker) {
  const sessionsPromise = firestore()
    .collection('sessions')
    .get()
  const schedulePromise = firestore()
    .collection('schedule')
    .orderBy('date', 'desc')
    .get()
  const speakersPromise = firestore()
    .collection('speakers')
    .get()
